    ...
    **kwargs,
    ...
    if transformer is None:
        if override_weights_file is not None:
            ...
            #note that in eval, load_state_dict is used - but it is an overkill
            if not torch.cuda.is_available():
                state = torch.load(override_weights_file, map_location=torch.device('cpu'))
            else:
                state = torch.load(override_weights_file)
            weights = state['algorithm'] #state_dict.model
            #HACK! but it seems to work!
            override_weights = {}
            for key in weights:
                #this removes the model part in the keys
                override_weights[key[6:]] = weights[key]
