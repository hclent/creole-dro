...
class GroupAlgorithm(Algorithm):
    ...
    def update_log(self, results, vocab_size):
        """
        ...
        with torch.no_grad():
            for m in self.logged_metrics:
                ...
                # print("update:", m.agg_metric_field)
                # print(batch_log[m.agg_metric_field])
    ...
    def get_pretty_log_str(self):
        ...
        for field in self.logged_fields:
            if field.startswith(self.group_prefix):
                continue
            results_str += (
                f'logged_fields {field}: {log[field]:.4f}\n'
            )

        # Process aggregate logged metrics
        for metric in self.logged_metrics:
            results_str += (
                f'logged_metrics {metric.agg_metric_field}: {log[metric.agg_metric_field]:.4f}\n'
            )
        ...
        if not self.no_group_logging:
            for g in range(self.grouper.n_groups):
                ...
                for metric in self.logged_metrics:
                    results_str += (
                        f'{metric.name}: '
                        f'{log[metric.group_metric_field(g)]:4.4f}\t'
                    )
                results_str += '\n'
        else:
            results_str += '\n'
