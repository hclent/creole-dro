...
class Metric:
    ...
    def _compute(self, y_pred, y_true, vocab_size):
        """
    ...
    def compute(self, y_pred, y_true, vocab_size, return_dict=True):
        """
        ...
        if y_true.numel()==0:
            ...
            agg_metric = self._compute(y_pred, y_true, vocab_size)
        if return_dict:
            results = {
    ...
    def compute_group_wise(self, y_pred, y_true, g, n_groups, vocab_size, return_dict=True):
        """
        ...
        """ #FIXME: add vocab size
        group_metrics, group_counts, worst_group_metric = self._compute_group_wise(y_pred, y_true, g, n_groups, vocab_size)
        if return_dict:
            results = {}
...
class ElementwiseMetric(Metric):
    ...
    def _compute(self, y_pred, y_true, vocab_size):
        ...
        """
        element_wise_metrics = self._compute_element_wise(y_pred, y_true, vocab_size)
        avg_metric = element_wise_metrics.mean()
        return avg_metric

    def _compute_group_wise(self, y_pred, y_true, g, n_groups, vocab_size): #FIXME GROUP SIZE
        element_wise_metrics = self._compute_element_wise(y_pred, y_true, vocab_size) #FIXME GROUP SIZE
        #print(f"elem_wise: {element_wise_metrics}")
        #print(f"g: {g}")
        #print(g.shape)
        new_g = g.repeat_interleave(y_true.shape[1])
        #new_g = g.view(-1)
        #print(new_g[0:50])
        #print(f"****************")
        #print(new_g[-50:-1])
        new_g = new_g[element_wise_metrics != 0]
        #print(f"new g: {new_g.shape}")
        #exit(1234)
        new_element_wise_metrics = element_wise_metrics[element_wise_metrics != 0]
        #print(f"elem wise size: {element_wise_metrics.shape}")
        #print(f"new dude: {new_element_wise_metrics.shape}") 
        group_metrics, group_counts = avg_over_groups(new_element_wise_metrics, new_g, n_groups)
        worst_group_metric = self.worst(group_metrics[group_counts>0])
        return group_metrics, group_counts, worst_group_metric
    ...
    def compute_element_wise(self, y_pred, y_true, vocab_size, return_dict=True):
        """
    ...
    def compute_flattened(self, y_pred, y_true, vocab_size, return_dict=True):
        flattened_metrics = self.compute_element_wise(y_pred, y_true, vocab_size, return_dict=False)
